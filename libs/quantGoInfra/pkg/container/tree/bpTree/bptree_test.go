...
func (v BPTreeValidator) Validate(nodeAccessor NodeAccessor) (_ error) {
...
func TestBPTreeAddRecord(t *testing.T) {
	...
	v, ok := bpt.AddRecord(KeyMin, nil)

	if assert.False(t, ok) {
		assert.Equal(t, SortedKeywordIndexes[0], v)
	}

	v, ok = bpt.AddRecord(KeyMax, nil)
...
func TestBPTreeUpdateRecord(t *testing.T) {
	...
	v, ok := bpt.UpdateRecord(KeyMin, nil)

	if assert.True(t, ok) {
		assert.Equal(t, SortedKeywordIndexes[0], v)
	}

	v, ok = bpt.UpdateRecord(KeyMax, nil)
...
func TestBPTreeDeleteRecord(t *testing.T) {
	...
	for h-l+1 > len(Keywords)/2 {
		v, ok := bpt.DeleteRecord(KeyMin)

		if assert.True(t, ok) {
			assert.Equal(t, SortedKeywordIndexes[l], v)
		}

		v, ok = bpt.DeleteRecord(KeyMax)
...
func TestBPTreeSearch(t *testing.T) {
	...
	for _, tmp := range [][2]interface{}{
		{KeyMin, KeyMax},
		{Keywords[minI], KeyMax},
		{KeyMin, Keywords[maxI]},
	...
	for _, tmp := range [][3]interface{}{
		{KeyMin, KeyMin, minI},
		{KeyMin, Keywords[minI], minI},
		{Keywords[minI], KeyMin, minI},
		{Keywords[minI], Keywords[minI], minI},
		{KeyMax, KeyMax, maxI},
		{KeyMax, Keywords[maxI], maxI},
		{Keywords[maxI], KeyMax, maxI},
	...
	{
		bpt := new(BPTree).Init(5, func(key1, key2 interface{}) int64 {
			return int64(strings.Compare(key1.(string), key2.(string)))
		})

		it := bpt.SearchForward(KeyMin, KeyMax)
		assert.True(t, it.IsAtEnd())
		it = bpt.SearchForward(KeyMax, KeyMin)
		assert.True(t, it.IsAtEnd())
		...
		it = bpt.SearchForward(KeyMin, "a ")
		assert.True(t, it.IsAtEnd())

		it = bpt.SearchForward("l", KeyMax)
		assert.True(t, it.IsAtEnd())
...
func MakeBPTree(t *testing.T, maxDegree int) *BPTree {
	bpt := new(BPTree).Init(maxDegree, func(key1, key2 interface{}) int64 {
		return int64(strings.Compare(key1.(string), key2.(string)))
	})
...
func TestMain(m *testing.M) {
	data, err := ioutil.ReadFile("./bitquark-subdomains-top100000.txt")
