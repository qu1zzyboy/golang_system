...
var (
	symObjArray []*BnSymbolSub256 // 交易对信息
...
func InitObjArr(size int, resourcePoolArr []resourceEnum.ResourceType) {
	...
	symObjArray = make([]*BnSymbolSub256, size)
	for i := range size {
		symObjArray[i] = newBnSymbolSub256(systemx.SymbolIndex16I(i))
	}
}

func GetSymbolObj(symbolIndex systemx.SymbolIndex16I) *BnSymbolSub256 {
