...
func (s *TradeManager) SetByBitLeverage(leverage uint8, symbolName string) error {
	...
	// 	})
	// 	if err != nil {
	// 		toUpbitList.DyLog.GetLog().Errorf("[%d]设置币安[%s]杠杆失败,leverage:%d,err:%v", k, symbolName, leverage, err)
	// 		return err
	// 	}
	// 	toUpbitList.DyLog.GetLog().Infof("[%d]设置币安[%s]杠杆成功,leverage:%d,res:%v", k, symbolName, leverage, res)
	// }
	return nil
}
