...
import (
	...
	"upbitBnServer/internal/quant/execute/order/orderSdk/bn/queryOrderBn"
...
func (s *FutureClient) QueryOrder(req orderModel.MyQueryOrderReq) error {
	...
	rawData := queryOrderBn.GetBnFu_NoSign_Q(req.SymbolName, reqId, req.ClientOrderId)
...
func (s *FutureClient) CancelOrder(req orderModel.MyQueryOrderReq) error {
	reqId := time2str.GetNowTimeStampMicroSlice16()
	rawData := queryOrderBn.GetBnFu_NoSign_C(req.SymbolName, reqId, req.ClientOrderId)
	if err := s.conn.WriteAsync(rawData); err != nil {
