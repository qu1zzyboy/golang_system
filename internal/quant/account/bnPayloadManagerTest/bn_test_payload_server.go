...
func (s *Payload) init(ctx context.Context, v accountConfig.Config, read wsDefine.ReadPrivateHandler) error {
	s.accountKeyId = v.AccountId
	s.payload = bnPayload.NewBnPayload(v.ApiKeyHmac, v.SecretHmac)
	if err := s.payload.RegisterReadHandler(ctx, v.AccountId, read); err != nil {
		return err
	}
	return nil
}
