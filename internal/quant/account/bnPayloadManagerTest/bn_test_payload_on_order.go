...
func (s *Payload) processPayloadOrderPrePoint(data []byte, clientOrderId string, symbolIndex int) {
	// 订单状态异常
...
func (s *Payload) onPayloadOrder(data []byte) {
	...
	if !ok {
		...
		if clientOrderId[0:5] == "point" {
			symbolIndex = bnVar.GetOrStoreNoTrade(gjson.GetBytes(data, "o.s").String())
			s.processPayloadOrderPrePoint(data, clientOrderId, symbolIndex)
			return
		}
