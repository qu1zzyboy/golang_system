...
func TestManualTreeNews(t *testing.T) {
	RegisterHandler(func(_ context.Context, evt Event) {
		t.Logf("exchange=%s exchange_type=%s symbols=%v id=%s", evt.Exchange, evt.ExchangeType.String(), evt.Symbols, evt.ID)
		if evt.ExchangeType != exchangeEnum.BINANCE {
			t.Fatalf("expected binance exchange type, got %s", evt.ExchangeType.String())
		}
	})
