...
func (m *Handle) Register(ctx context.Context, instanceId instanceEnum.Type, fields map[string]string, handler UpBitListDelta) error {
	fields[defineJson.From] = "to_UpBit_MeshHandle"
	return m.handlers.RegisterOrReplace(ctx, instanceId, fields, handler)
}

func (m *Handle) UnRegister(ctx context.Context, instanceId instanceEnum.Type, fields map[string]string) error {
	return m.handlers.Unregister(ctx, instanceId, fields)
}

func (m *Handle) OnSymbolList(ctx context.Context, mesh *coinMesh.CoinMesh) {
	m.handlers.Range(func(s instanceEnum.Type, delta UpBitListDelta) bool {
