...
import (
	...
	"upbitBnServer/internal/quant/exchanges/bybit/marketSub/bybitAutoMarketSub"
	"upbitBnServer/internal/quant/exchanges/bybit/marketSub/bybitPoolMarketSub"
...
func (e *Engine) OnSymbolList(ctx context.Context, s *coinMesh.CoinMesh) error {
	...
	// 再订阅
	if err := bybitPoolMarketSub.GetSymbolObj(symbolIndex).RegisterReadHandler(ctx, symbolName); err != nil {
		toUpBitDataStatic.DyLog.GetLog().Errorf("bn_upbit上币失败,err:%v", err)
		return err
	}
	if err := bybitAutoMarketSub.GetSymbolObj(symbolIndex).RegisterReadHandler(ctx, symbolName); err != nil {
		toUpBitDataStatic.DyLog.GetLog().Errorf("bn_upbit上币失败,err:%v", err)
		return err
	}
	return nil
...
func (e *Engine) OnSymbolDel(ctx context.Context, s *coinMesh.CoinMesh) error {
	...
	if rawIndex, ok := bybitVar.SymbolIndex.Load(s.BybitFuUsdtName); ok {
		bybitPoolMarketSub.GetSymbolObj(rawIndex).CloseSub(ctx)
		bybitAutoMarketSub.GetSymbolObj(rawIndex).CloseSub(ctx)
	}
	return nil
}
...
func (e *Engine) OnStop(ctx context.Context) error {
	bybitVar.SymbolIndex.Range(func(symbolName string, symbolIndex systemx.SymbolIndex16I) bool {
		bybitPoolMarketSub.GetSymbolObj(symbolIndex).CloseSub(ctx)
		bybitAutoMarketSub.GetSymbolObj(symbolIndex).CloseSub(ctx)
		return true
	})
...
func (e *Engine) OnDayBegin() {
	...
	if weekday == time.Saturday || weekday == time.Sunday {
		...
		bybitPoolMarketSub.OpenSub(ctx, stopIndex)
		bybitAutoMarketSub.OpenSub(ctx, stopIndex)
...
func (e *Engine) OnDayEnd() {
	ctx := context.Background()
	stopIndex := bybitVar.SymbolIndex.Length()
	bybitPoolMarketSub.CloseSub(ctx, stopIndex)      //关闭订阅
	bybitAutoMarketSub.CloseSub(ctx, stopIndex)      //关闭订阅
	time.Sleep(3 * time.Second)                      //等待数据处理完
