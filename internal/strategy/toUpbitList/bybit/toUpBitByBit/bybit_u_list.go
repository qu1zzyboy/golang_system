...
func (e *Engine) start(ctx context.Context, req *Req) error {
	...
	toUpbitParam.SetParam(req.Qty, req.Dec003, req.Dec500, req.AccountLen)
	// --- IGNORE ---
	...
	//chan对象数组
	bybitPoolMarketChan.InitChanArr(needLen)
	bybitAutoMarketChan.InitChanArr(needLen)
	toUpbitListChan.InitChanArr(needLen)

	//obj对象数组
	toUpbitBybitSymbolArr.InitObjArr(needLen)
	bybitPoolMarketSub.InitObjArr(needLen, []resourceEnum.ResourceType{resourceEnum.BOOK_TICK})
	bybitAutoMarketSub.InitObjArr(needLen, []resourceEnum.ResourceType{resourceEnum.AGG_TRADE, resourceEnum.MARK_PRICE})
	...
	//初始化各个行情数据引擎
	bybitPoolMarketSub.Register(ctx, symbols)
	bybitAutoMarketSub.Register(ctx, symbols)
