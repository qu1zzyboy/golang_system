...
const (
	...
	from_bn   = instanceEnum.TO_UPBIT_LIST_BN
)
...
func (s *PointPre) CheckPreOrder(symbolName string, markPrice float64, pScale systemx.PScale, qScale systemx.QScale) {
	if !s.hasInit {
		...
		//下小订单
		if err := bnOrderAppManager.GetTradeManager().SendPlaceOrder(s.preAccountKeyId, orderModel.MyPlaceOrderReq{
	...
	if diff >= 0.01*s.lastMarkPrice {
		...
		if _, ok := clientOrderOnline.Load(s.clientOrderIdSmall); ok {
			...
			//更新订单价格
			if err := bnOrderAppManager.GetTradeManager().SendModifyOrder(s.preAccountKeyId, orderModel.MyModifyOrderReq{
				SymbolName:    symbolName,
				ClientOrderId: s.clientOrderIdSmall,
				Pvalue:        smallPrice,
				Qvalue:        s.PointNum,
...
func (s *PointPre) CancelPreOrder(can *bnOrderTemplate.CancelTemplate, reqFrom instanceEnum.Type) {
	if !s.hasInit {
		return
	}
	can.RefreshClientOrderId(s.clientOrderIdSmall)
	bnOrderAppManager.GetTradeManager().SendCancelOrderBy(can, reqFrom, point_pre, s.preAccountKeyId)
}
