...
import (
	...
	"upbitBnServer/internal/strategy/newsDrive/bn/toUpBitListBnAccount"
	"upbitBnServer/internal/strategy/newsDrive/bn/toUpBitListBnMarket"
	...
	"upbitBnServer/internal/strategy/toUpbitList/bn/bnDriveSymbol"
...
func (e *Engine) start(ctx context.Context, req *Req) error {
	driverStatic.SetParam(req.PriceRiceTrig, req.TickCap, req.Dec500)
	driverStatic.ExType = exchangeEnum.BINANCE
	driverStatic.AcType = exchangeEnum.FUTURE
	bnDriveSymbol.SetParam(req.Qty, req.Dec003)
	// --- IGNORE ---
	...
	if len(symbols) == 0 {
		return driverStatic.ExType.GetNotSupportError("no symbols to subscribe")
	}
	driverStatic.DyLog.GetLog().Infof("bn初始化订阅币种:%d,max:%d", len(symbols), len(data))
	needLen := len(symbols) + 100
	driverListChan.InitUpBit(needLen)
	bnDriveSymbolArr.Init(needLen)

	for index, symbolName := range symbols {
		// 提前指定每个品种的挂单账户
		if err := bnDriveSymbolArr.GetSymbolObj(index).Start(e.getPreAccountKeyId(), index, symbolName); err != nil {
			return err
		}
	}
	// 注册行情资源
	...
	// 注册交易时间段
	if _, err := globalCron.AddFunc(driverStatic.DAY_BEGIN_STR, e.OnDayBegin); err != nil {
		return err
	}
	if _, err := globalCron.AddFunc(driverStatic.DAY_END_STR, e.OnDayEnd); err != nil {
		return err
	}
