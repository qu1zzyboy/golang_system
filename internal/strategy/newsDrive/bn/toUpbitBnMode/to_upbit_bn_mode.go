...
func (l LiveMode) GetTakeProfitParam(isMeme bool, symbolIndex int, cap float64) (float64, float64, error) {
	resp, err := driverParam.GetService().Compute(context.Background(), driverParam.ComputeRequest{
		IsMeme:      isMeme,
	...
	driverStatic.DyLog.GetLog().Debugf("param service result: symbol=%d gain=%.2f twap=%.2f", symbolIndex, resp.GainPct, resp.TwapSec)
