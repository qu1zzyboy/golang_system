...
func (s *Single) onPayloadOrder(data []byte) {
	if toUpBitListDataAfter.LoadTrig() {
		if s.symbolIndex == toUpBitListDataAfter.TrigSymbolIndex {
			driverStatic.SendToUpBitMsg("发送bn二次确认失败", map[string]string{
				"symbol": s.StMeta.SymbolName,
				"op":     "ORDER_UPDATE二次上涨确认",
			})
			driverStatic.DyLog.GetLog().Infof("触发后二次确认:%s", s.StMeta.SymbolName)
		}
	} else {
		/*********************上币还未触发**************************/
		eventTs := gjson.GetBytes(data, jsonEvent).Int()
		go func() {
			driverStatic.SigLog.GetLog().Infof("[%d]触发前成交,%s", s.preAccountKeyId, string(data))
			driverStatic.SendToUpBitMsg("发送bn预挂单成交失败", map[string]string{
				"symbol": s.StMeta.SymbolName,
