...
func (s *Single) checkPreOrder(markPrice_8 uint64) {
	if !s.hasInit {
		...
		if err := s.initPreOrder(); err != nil {
			driverStatic.DyLog.GetLog().Errorf("%s 初始化交易对失败:  %s", s.StMeta.SymbolName, err.Error())
			return
		}
		//下小订单
		...
		if err := bnOrderAppManager.GetTradeManager().SendPlaceOrder(ws_req_from, s.preAccountKeyId, s.symbolIndex,
			...
			driverStatic.DyLog.GetLog().Errorf("创建小订单错误: %s", err.Error())
		}
		return
	}
	...
	if u64Cal.IsDiffMoreThanPercent100(markPrice_8, s.lastMarkPrice_8, 1) {
		...
		if _, ok := clientOrders.Load(s.clientOrderIdSmall); ok {
			driverStatic.DyLog.GetLog().Infof("[%d,%s] 触发[%d,%d,%d_%d],准备更新预挂单:%s",
			...
			if err := bnOrderAppManager.GetTradeManager().SendModifyOrder(ws_req_from, s.preAccountKeyId,
				...
				driverStatic.DyLog.GetLog().Errorf("%s修改小订单错误: %s", symbolKey, err.Error())
			}
		} else {
			driverStatic.DyLog.GetLog().Infof("[%d,%s] 触发[%d,%d,%d],准备重下预挂单:%s", s.preAccountKeyId, symbolKey, s.lastMarkPrice_8, markPrice_8, s.pScale, modifySmallPrice)
			if _, ok := clientOrderSig.Load(s.clientOrderIdSmall); ok {
				driverStatic.DyLog.GetLog().Infof("订单限流中,无法下单:%s", s.clientOrderIdSmall)
				return
			}
			//下小订单
			if err := bnOrderAppManager.GetTradeManager().SendPlaceOrder(ws_req_from, s.preAccountKeyId, s.symbolIndex,
				...
				driverStatic.DyLog.GetLog().Errorf("%s创建小订单错误: %s", symbolKey, err.Error())
			}
		}
	}
}
