...
func (s *Single) receiveStop(stopType StopType) {
	...
	driverStatic.DyLog.GetLog().Infof("收到停止信号==> %s", stopReasonArr[stopType])
	s.cancel()
	//开启平仓线程
	safex.SafeGo("to_upbit_bn_close", func() {
		defer func() {
			driverStatic.DyLog.GetLog().Infof("当前账户id[%d] 平仓协程结束", s.thisOrderAccountId.Load())
		...
		if use.LessThanOrEqual(decimal.Zero) {
			driverStatic.DyLog.GetLog().Infof("没有可用的平仓数量,取消平仓")
			return
		}
		if use.LessThanOrEqual(driverStatic.Dec500) {
			driverStatic.DyLog.GetLog().Infof("没有足够的平仓数量,取消平仓")
			return
		}
		//每秒平一次
			...
			case <-ticker.C:
				{
					...
					if s.pos.GetTotal().Mul(priceDec).LessThanOrEqual(driverStatic.Dec500) {
						driverStatic.DyLog.GetLog().Infof("平仓完全成交,开始清理资源")
						ticker.Stop()
						return
					}
					driverStatic.DyLog.GetLog().Infof("============开始平仓,剩余:%s============", posLeft)
					// 最新的每个账户的仓位情况
					copyMap := s.pos.GetAllAccountPos()
					for accountKeyId, vol := range copyMap {
						// 已经完全平完了
						...
						if err := bnOrderAppManager.GetTradeManager().SendPlaceOrder(order_from, accountKeyId, s.symbolIndex,
							&orderModel.MyPlaceOrderReq{
								OrigPrice:     priceDec,
								OrigVol:       num,
								ClientOrderId: driverStatic.GetClientOrderIdBy("close"),
								StaticMeta:    s.StMeta,
								OrderType:     execute.ORDER_TYPE_LIMIT,
								OrderMode:     execute.ORDER_SELL_CLOSE,
							}); err != nil {
							driverStatic.DyLog.GetLog().Errorf("每秒平仓创建订单失败: %v", err)
						}
					}
				}
			case <-timeout:
				driverStatic.DyLog.GetLog().Infof("平仓时间结束,开始清理资源")
				ticker.Stop()
				return
			}
		}
