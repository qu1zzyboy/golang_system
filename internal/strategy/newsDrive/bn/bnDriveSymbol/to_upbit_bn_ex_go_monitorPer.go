...
func (s *Single) monitorPer(accountIndex uint8) {
	var i int
	defer func() {
		driverStatic.DyLog.GetLog().Infof("账户[%d],探测[%d]次,协程结束", accountIndex, i)
	}()
...
OUTER:
	for i = 0; i <= 230; i++ {
		select {
		case <-s.ctxStop.Done():
			driverStatic.DyLog.GetLog().Infof("收到关闭信号,退出探测协程")
			break OUTER
		default:
			//有成交或者本轮挂单成功
			...
			if err := bnOrderAppManager.GetMonitorManager().SendMonitorOrder(order_from, accountIndex, s.symbolIndex,
				&orderModel.MyPlaceOrderReq{
					...
					ClientOrderId: driverStatic.GetClientOrderIdBy("sec-Mo"),
					StaticMeta:    s.StMeta,
					OrderType:     execute.ORDER_TYPE_LIMIT,
					OrderMode:     execute.ORDER_BUY_OPEN,
				}); err != nil {
				driverStatic.DyLog.GetLog().Errorf("每秒探测订单失败: %v", err)
			}
