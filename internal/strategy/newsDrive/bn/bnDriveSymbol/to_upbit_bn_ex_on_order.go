...
func (s *Single) onSuccessOrder(evt toUpBitListDataAfter.OnSuccessEvt) {
	...
	if evt.IsOnline {
		...
		if evt.Volume.GreaterThan(decimal.Zero) {
			...
			if evt.OrderMode.IsOpen() {
				...
				if val != nil {
					...
					if left.Mul(lastPrice).LessThan(driverStatic.Dec500) {
						s.hasAllFilled.Store(true)
						driverStatic.DyLog.GetLog().Infof("完全成交,当前总仓位:%s,需要:%s", posTotalAmount, s.posTotalNeed)
					}
				}
				driverStatic.SendToUpBitMsg("发送开仓成交失败", map[string]string{
				...
				driverStatic.SendToUpBitMsg("发送平仓成交失败", map[string]string{
					"symbol": s.StMeta.SymbolName,
		...
		driverStatic.DyLog.GetLog().Infof("账户[%d][%s]订单完成[%s],剩余挂单数:%d",
