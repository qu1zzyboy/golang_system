...
import (
	...
	"sync/atomic"
	"time"
	"upbitBnServer/internal/strategy/newsDrive/bn/bnBeforePointer"
	...
	"upbitBnServer/internal/strategy/newsDrive/common/driverSymbol"
...
type cache_line_4 struct {
	...
	chanMarkPrice      chan driverListChan.Job // 标记价格chan
...
type cache_line_5 struct {
	...
	chanBookTick chan driverListChan.Job // 盘口数据chan
...
type Single struct {
	...
	pre            *bnBeforePointer.PointPreBN // 5%预挂单对象
	cache_line_1
	cache_line_2
	cache_line_3
	cache_line_4
...
func (s *Single) Clear() {
	s.newPriceMinWindowU64(driverStatic.TickCap)
	s.lastRiseValue = 0.0
}
...
func (s *Single) Start(accountKeyId uint8, index int, symbolName string) error {
	...
	if !ok {
		driverStatic.DyLog.GetLog().Errorf("symbolId not found: %s", symbolName)
		return driverStatic.ExType.GetNotSupportError("symbolId not found")
	}
	symbolKeyId := symbolInfo.MakeSymbolKey3(driverStatic.ExType, driverStatic.AcType, symbolId)
	// 初始化品种静态数据
	stMeta, err := symbolStatic.GetTrade().Get(symbolKeyId)
	if err != nil {
		driverStatic.DyLog.GetLog().Errorf("%s symbolKeyId %d not found", symbolName, symbolKeyId)
		return err
	}
	s.StMeta = &stMeta
	// 初始化品种动态数据
	...
	if err != nil {
		driverStatic.DyLog.GetLog().Errorf("symbolKeyId %d not found", symbolKeyId)
		return err
	}
	...
	s.newPriceMinWindowU64(driverStatic.TickCap)
	s.chanBookTick = make(chan driverListChan.Job, 100)
	s.chanAggTrade = make(chan driverListChan.Job, 10)
	s.chanMarkPrice = make(chan driverListChan.Job, 10)
	s.chanTradeLite = make(chan []byte, 10)
	s.chanOrderUpdatePre = make(chan []byte, 10)
	s.chanMonitor = make(chan []byte, 10)
	s.chanOutSideSig = make(chan driverListChan.Special, 10)
	s.chanSuOrder = make(chan toUpBitListDataAfter.OnSuccessEvt, 10)
	driverListChan.RegisterMarket(index, s.chanBookTick, s.chanAggTrade, s.chanMarkPrice)
	driverListChan.RegisterPrivate(index, s.chanTradeLite, s.chanOrderUpdatePre, s.chanMonitor, s.chanOutSideSig, s.chanSuOrder)
	...
	s.symbolIndex = index
	driverStatic.SymbolIndex.Store(symbolName, index)
	for i := range 11 {
		temp := &secondPerInfo{}
		temp.clear()
		s.secondArr[i] = temp
...
func (s *Single) onLoop() {
	for {
		...
		case sig := <-s.chanOutSideSig:
			{
				switch sig.SigType {
				case driverListChan.ReceiveTreeNews:
					{
						s.ReceiveTreeNews()
					}
				case driverListChan.ReceiveNoTreeNews:
					{
						s.ReceiveNoTreeNews()
					}
				case driverListChan.ReceiveTreeNewsAndOpen:
					{
						s.intoExecuteByMsg()
					}
				case driverListChan.CANCEL_ORDER_RETURN:
					{
						s.onCanceledOrder(sig.AccountKeyId)
					}
				case driverListChan.FailureOrder:
					{
						s.onFailureOrder(sig.AccountKeyId, sig.ErrCode)
					}
				case driverListChan.QUERY_ACCOUNT_RETURN:
					{
...
func (s *Single) onMonitorData(data []byte) {
	...
	driverStatic.DyLog.GetLog().Infof("最新探测价格[%d]: %s", monitor_10, errMsg)
}
