...
func (s *Single) placePer(i int32, accountIndex uint8) {
	...
	defer func() {
		driverStatic.DyLog.GetLog().Infof("账户[%d_%d],抽奖[总:%d,maker:%d,limit:%d,market:%d]次,上限[%s],协程结束",
...
OUTER:
	for j = 0; j <= 230; j++ {
		select {
		case <-s.ctxStop.Done():
			driverStatic.DyLog.GetLog().Infof("收到关闭信号,退出每秒下单协程")
			break OUTER
		default:
			//有成交或者本轮挂单成功
			...
			if toUpbitBnMode.Mode.ShouldExitOnTakeProfit(priceBuy.InexactFloat64(), s.takeProfitPrice) {
				driverStatic.DyLog.GetLog().Infof("超出止盈价格:[买入价:%.8f,止盈价:%.8f],退出每秒下单协程", priceBuy.InexactFloat64(), s.takeProfitPrice)
				return
			}
			...
			if err := bnOrderAppManager.GetTradeManager().SendPlaceOrder(order_from, accountIndex, s.symbolIndex,
				&orderModel.MyPlaceOrderReq{
					...
					ClientOrderId: driverStatic.GetClientOrderIdBy("second"),
					StaticMeta:    s.StMeta,
					OrderType:     orderType,
					OrderMode:     execute.ORDER_BUY_OPEN,
				}); err != nil {
				driverStatic.DyLog.GetLog().Errorf("每秒创建订单失败: %v", err)
			}
