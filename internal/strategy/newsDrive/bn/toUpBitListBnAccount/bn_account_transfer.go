...
func (s *BnAccountManager) TransferIn(from int32, amount decimal.Decimal) error {
	...
	if usdtAmount.LessThan(decimal.Zero) {
		driverStatic.DyLog.GetLog().Infof("账户[%d]余额不足200u,不划转", out.AccountId)
		return nil
	}
	...
	if err != nil {
		driverStatic.DyLog.GetLog().Errorf("[%d]划转到母账户[%s]失败,err:%s,请求参数:%s", out.AccountId, reqIn.ToAcType, err.Error(), reqParamIn)
		return err
	}
	driverStatic.DyLog.GetLog().Infof("账户[%d]划转[%s,%s]usdt到母账户[%s]成功:%s", out.AccountId, amount, reqIn.Amount, reqIn.ToAcType, resIn)
	return nil
}
...
func (s *BnAccountManager) TransferOut(to int32, amount decimal.Decimal) error {
	...
	if err != nil {
		driverStatic.DyLog.GetLog().Errorf("母账户划转到[%d]失败,err:%s,请求参数:%s", in.AccountId, err.Error(), reqParamOut)
		return err
	}
	if gjson.Get(resOut, "tranId").Exists() {
		driverStatic.DyLog.GetLog().Infof("母账户划转[%s]usdt到[%d][%s]成功:%s", reqOut.Amount.String(), in.AccountId, reqOut.ToAcType, resOut)
		return nil
	}
...
func (s *BnAccountManager) RefreshSymbolConfig() error {
	...
	gjson.ParseBytes(data).ForEach(func(key, value gjson.Result) bool {
		symbolIndex, ok := driverStatic.SymbolIndex.Load(value.Get("symbol").String())
		if !ok {
			return true
		}
		driverStatic.SymbolMaxNotional.Store(symbolIndex, decimal.RequireFromString(value.Get("maxNotionalValue").String()))
		return true
	})
	driverStatic.DyLog.GetLog().Infof("共刷新%d个交易对开仓上限信息", driverStatic.SymbolMaxNotional.Length())
	return nil
}
