...
type BnKline struct {
	...
	interval klineEnum.Interval
}

func NewBnKline(interval klineEnum.Interval) *BnKline {
	return &BnKline{
		interval: interval,
	}
}

func (s *BnKline) DialToMarket(ctx context.Context, params []string) (*wsDefine.SafeWrite, error) {
...
func (s *BnKline) ParamBuild(resourceType resourceEnum.ResourceType) (func(symbol string) string, error) {
	switch resourceType {
...
func (s *BnKline) AddSub(ctx context.Context, params []string) error {
	// 1、获取当前的雪花ID
	var snowId int64
...
func (s *BnKline) ListSub(ctx context.Context) error {
	// 1、获取当前的雪花ID
	var snowId int64
	if err := tracex.WithTrace(ctx, true, s.getTraceId(defineOp.OP_GET_SNOWFLAKE_ID), func(ctx context.Context) error {
		id, err := idGen.GetSnowIdInt64()
...
func (s *BnKline) UnSub(ctx context.Context, params []string) error {
	// 1、获取当前的雪花ID
	var snowId int64
	if err := tracex.WithTrace(ctx, true, s.getTraceId(defineOp.OP_GET_SNOWFLAKE_ID), func(ctx context.Context) error {
		id, err := idGen.GetSnowIdInt64()
...
func (s *BnKline) sendWsData(ctx context.Context, op string, jsonBytes []byte) error {
	return tracex.WithTrace(ctx, true, op, func(ctx context.Context) error {
		dynamicLog.Log.GetLog().Infof("BnKline: %s", string(jsonBytes))
		return s.conn.SafeWriteMsg(websocket.TextMessage, jsonBytes)
	})
}

func (s *BnKline) getTraceId(op string) string {
	return idGen.BuildName2("BnKline", op)
}
